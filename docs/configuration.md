...
The *node-iconizr*'s **main configuration** is provided to the [constructor](api.md#iconizr-config-) as an `Object` with the following structure:
...
If you are familiar with the underlying [svg-sprite](https://github.com/jkphl/svg-sprite), you may notice that all properties but `icons` are inherited from there. The **properties shared between svg-sprite and node-iconizr** are described in detail in the [svg-sprite configuration documentation](https://github.com/jkphl/svg-sprite/blob/master/docs/configuration.md) — basically all of them apply to *node-iconizr* as well. This also means that it's possible to [leverage even more of svg-sprite's features](../README.md#leveraging-more-svg-sprite-features) than those ones documented here.

What follows is a complete reference of all **icon specific configuration settings**. ~~For getting off the ground quickly, you may also use the [online configurator & kickstarter](http://jkphl.github.io/node-iconizr), which lets you create a custom configuration in seconds~~ *[Not available yet]*.


Table of contents
-----------------

* [Icon properties overview](#icon-properties-overview)


### Icon properties overview

As all the other configuration properties, All the configuration properties are optional, so even an empty object `{}` is a valid configuration for *node-iconizr*. 
