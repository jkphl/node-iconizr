...
Iconizr.prototype._buildDataUrisAndPaths = function(files, data, callback) {
	...
	data[this.key].shapes.map(function(shape, index){
		...
		shape.__defineGetter__('svgDataUri', function() {
			return 'data:image/svg+xml,' + encodeURIComponent(that._standalonize(shape.svg));
		});
...
Iconizr.prototype._buildStylesheetResourcePayloads = function(config, data, fallback, callback) {
	...
	for (var extension in config) {
		...
		for (var strategy in strategyList) {
			...
			tasks.push(function(extension, template, prefix, dest, strategy, bust, resource){
				var localDest			= dest + '.' + strategy + (bust ? '%s' : '') + '.' + extension,
				localCssDest			= path.dirname(((extension == 'css') ? config[extension].realdest : false) || cssDest),
				...
				localData.icon			= function(content, context) {
					...
					return isDataUri ? ('"' + icon + '"') : path.relative(localCssDest, path.resolve(that.config.dest, icon));
			    };
			...
			}(extension, template, prefix, dest, strategy, bust, (realdest == config[extension].dest) ? extension : 'preview'));
		}
	}
...
Iconizr.prototype._buildLoaderResource = function(files, data, callback) {
	...
	if (_.size(this.config.icons.render)) {
		...
		var loader						= this._buildLoaderResourcePayload(this._spriter.config.dest, this.config.icons.loader, data.exceed);
		if (loader) {
 ...
 */
Iconizr.prototype._buildLoaderResourcePayload = function(dest, config, exceed) {
	var data						= {
		'fallback-sprite'			: util.format(config.css, 'fallback-sprite'),
		'fallback-data-uri'			: util.format(config.css, exceed.fallback ? 'fallback-sprite' : 'fallback-data-uri'),
		'svg-sprite'				: util.format(config.css, 'svg-sprite'),
		'svg-data-uri'				: util.format(config.css, exceed.svg ? 'svg-sprite' : 'svg-data-uri'),
		'embed'						: config.embed
	},
	payload							= mustache.render(fs.readFileSync(path.join(process.cwd(), 'tmpl', 'loader.js'), 'utf-8'), data);
...
Iconizr.prototype._buildHTMLPreview = function(files, data, callback) {
	...
	if (this.config.icons.preview) {
		...
		stylesheets					= {
			...
			'fallback-image'		: ['Fallback images'		, path.join(embed, util.format(css, 'fallback-image'))],
			'fallback-sprite'		: ['Fallback sprite'		, path.join(embed, util.format(css, 'fallback-sprite'))],
			'fallback-data-uri'		: ['Fallback data URIs'		, path.join(embed, util.format(css, 'fallback-data-uri'))],
			'svg-image'				: ['SVG images'				, path.join(embed, util.format(css, 'svg-image'))],
			'svg-sprite'			: ['SVG sprite'				, path.join(embed, util.format(css, 'svg-sprite'))],
			'svg-data-uri'			: ['SVG data URIs'			, path.join(embed, util.format(css, 'svg-data-uri'))]
		},
		previewTemplate				= fs.readFileSync(path.join(path.dirname(__dirname), 'tmpl', 'preview.html'), 'utf-8'),
		loader						= this._buildLoaderResourcePayload(this._spriter.config.dest, {
			type					: 'html',
		...
		for (var s1 in stylesheets) {
			var dest1				= path.normalize(path.join(this.config.icons.preview, path.basename(stylesheets[s1][1] || util.format(css, 'auto'), '.css') + '-preview.html')),
			localData					= {
			...
			for (var s2 in stylesheets) {
				var dest2			= path.basename(stylesheets[s2][1] || util.format(css, 'auto'), '.css') + '-preview.html';
				localData.nav.push({
...
/**
 * Add necessary base namespaces to a standalone SVG
 * 
 * @param {String} svg			SVG
 * @return {String}				SVG with namespaces
 */
Iconizr.prototype._standalonize = function(svg) {
	var root						= svg.match(/\<svg.*?\>/i);
	if (_.isArray(root)) {
		var parts					= [root.index ? svg.substr(0, root.index) : '', svg.substr(root.index + root[0].length)];
		root						= root[0].substr(5);
		if (parts[1].indexOf(' xlink:href=') >= 0) {
			root					= 'xmlns:xlink="http://www.w3.org/1999/xlink" ' + root;
		}
		if (root.indexOf(' xmlns=') < 0) {
			root					= 'xmlns="http://www.w3.org/2000/svg" ' + root;
		}
		svg							= parts.join('<svg ' + root);
	}	
	return svg;
}
