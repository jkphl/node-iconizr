...
function Iconizr(result) {
	...
	this._options.loader		= _.extend({type: 'html', dest: '.', minify: true}, this._options.loader || {});
...
Iconizr.prototype._svg2png = function(callback) {
	...
	async.parallelLimit(tasks, os.cpus().length * 2, function(error) { callback(error); });
}
...
Iconizr.prototype._renderLoaderFragment = function(callback) {
	...
	if (this.output) {
		...
		try {
			var that								= this,
			type									= (['html', 'js'].indexOf(this._options.loader.type) >= 0) ? this._options.loader.type : 'html',
			dest									= path.join(path.resolve(this._options.outputDir, this._options.loader.dest), this.css + '-loader-fragment.' + type),
			...
			script									= mustache.render(fs.readFileSync(path.join(path.dirname(__dirname), 'tmpl', 'loader.js'), 'utf-8'), data).split('##embed##').join(embed);
			
			// Optionally minify loader script
			if (!!this._options.loader.minify) {
				script								= UglifyJS.minify(script, {fromString: true}).code;
			}
			
			// Output loader as JavaScript file
			if (type === 'js') {
			...
			// Else: Output loader as HTML fragment
			} else {
				embed								= (!embed || (embed == '.')) ? '' : (embed + '/');
				data.script							= script;
				var html 							= mustache.render(fs.readFileSync(path.join(path.dirname(__dirname), 'tmpl', 'loader.html'), 'utf-8'), data).split('##embed##').join(embed);
				if (html.length) {
