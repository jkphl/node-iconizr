...
Iconizr.prototype._buildHTMLPreview = function(files, data, callback) {
	...
	if (this.config.icons.preview) {
		...
		var that					= this,
		tasks						= {},
		realdest					= this.config.icons.render.css.realdest || this.config.icons.render.css.dest,
		embed						= path.relative(this.config.icons.preview, path.dirname(path.resolve(this.config.dest, realdest))) || '.',
		css							= realdest.replace(/\.[^\.]+$/, '.%s$&'),
		stylesheets					= {
		...
		if (this.config.icons.dimensions && (this.config.icons.dimensions !== true)) {
			var dims				= path.basename(util.format(css, 'dimensions'));
		...
		for (var s1 in stylesheets) {
			...
			localData				= {
				...
				dims				: stylesheets[s1][1] ? false : dims,
				classname           : function() {
