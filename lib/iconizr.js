...
Iconizr.prototype._buildStylesheetResourcePayloads = function(config, data, fallback, callback) {
	...
	for (var extension in config) {
		...
		realdest						= config[extension].realdest || config[extension].dest,
		prefix							= path.basename(realdest, path.extname(realdest)),
		dest							= path.resolve(this.config.dest, path.join(path.dirname(realdest), prefix)),
		...
		for (var strategy in strategyList) {
			...
			tasks.push(function(extension, template, prefix, dest, strategy, bust, resource){
				...
				localData.sprite		= sprites[strategy] ? path.relative(localCssDest, sprites[strategy]) : null;
			    ...
			    return function(_cb) {
					...
					_cb(null, file, strategy, resource);
				}
...
Iconizr.prototype._buildLoaderResource = function(files, data, callback) {
	...
	if (_.size(this.config.icons.render)) {
		...
		var loader						= this._buildLoaderPayload(this._spriter.config.dest, this.config.icons.loader, data.exceed);
		if (loader) {
 ...
 */
Iconizr.prototype._buildLoaderPayload = function(dest, config, exceed) {
	var data						= {
		'png-sprite'				: util.format(config.css, 'png-sprite'),
		'png-data-uri'				: util.format(config.css, exceed.png ? 'png-sprite' : 'png-data-uri'),
